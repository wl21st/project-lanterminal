...
public class LanterminalEngine {
    ...
    public void drawUI(String header, String footer) throws IOException {
        ...
        screen.newTextGraphics().drawLine(1, 1, width - 1, 1, TextCharacter.fromCharacter(Symbols.DOUBLE_LINE_HORIZONTAL)[0]
        ...
        screen.newTextGraphics().drawLine(1, height - 2, width - 1, height - 2, TextCharacter.fromCharacter(Symbols.DOUBLE_LINE_HORIZONTAL)[0]
                .withForegroundColor(ANSI.YELLOW));
    ...
    public void typeln(String msg, int col, int row) {
        ...
        for (int i = 0; i < msg.length(); i++) {
            ...
            try {
                screen.refresh();
                TimeUnit.MILLISECONDS.sleep(ThreadLocalRandom.current().nextInt(interval * 3));
            } catch (InterruptedException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            } catch (IOException e) {
